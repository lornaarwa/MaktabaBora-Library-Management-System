...
import { BookOpen, Clock, AlertTriangle, ShieldCheck, Sparkles, CreditCard, ShoppingBag, ExternalLink, Loader2 } from 'lucide-react';
...
export default function MemberDashboard() {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            setLoading(true);
            try {
                const [loansRes, subRes] = await Promise.all([
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 shadow-xl">
                <div className="space-y-2">
                    <div className="flex items-center gap-3">
                        <h1 className="text-2xl sm:text-3xl font-black text-white">{user?.name || 'Patron Dashboard'}</h1>
                        {isSubscribed ? (
                ...
                {!isSubscribed && (
                    ...
                    >
                        <Sparkles className="w-4 h-4" /> Get Pro Perk Pass (KES 500/mo)
                    </button>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 flex items-center gap-4">
                    ...
                    <div>
                        <span className="text-xs text-slate-400 block font-medium">Outstanding Fines</span>
                        <span className="text-2xl font-black text-emerald-400">KES 0.00</span>
                    </div>
                </div>
            </div>
