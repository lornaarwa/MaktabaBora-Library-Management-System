    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
            ...
            {errorMsg && (
                <div className="p-3.5 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-200 text-xs flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <AlertCircle className="w-4 h-4 text-zinc-400" /> {errorMsg}
                    </div>
                    <button onClick={() => setErrorMsg(null)}><X className="w-4 h-4 text-zinc-400" /></button>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className={`${isSidebarCollapsed ? 'lg:col-span-1' : 'lg:col-span-3'} bg-zinc-900 border border-zinc-800 rounded-2xl p-3.5 space-y-2 h-fit shadow-sm transition-all`}>
                    <div className="flex items-center justify-between px-2 py-1">
                        {!isSidebarCollapsed && (
                            <h3 className="text-[10px] font-mono font-extrabold text-zinc-500 uppercase tracking-wider flex items-center gap-2">
                                <Table className="w-3.5 h-3.5 text-zinc-400" /> Desk Navigation
                            </h3>
                        )}
                        <button
                            onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}
                    ...
                    <div className="space-y-1">
                        ...
                        >
                            <div className="flex items-center gap-2.5">
                                <BookCheck className="w-4 h-4 text-zinc-400" />
                ...
                <div className={`${isSidebarCollapsed ? 'lg:col-span-11' : 'lg:col-span-9'} space-y-6 transition-all`}>
                    ...
                    {activeTab === 'overview' && (
                        <div className="space-y-6">
                            ...
                            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4 shadow-sm">
                                ...
                                {loading ? (
                                    <div className="flex items-center justify-center py-8 text-zinc-400 font-mono text-xs">
                        ...
                        <div className="space-y-6">
                            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4 shadow-sm">
                                <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
                                    <div>
                                        <h2 className="text-base font-bold text-zinc-100 flex items-center gap-2 font-mono">
                                            <BookOpen className="w-4 h-4 text-zinc-300" /> {editingBook ? 'Edit Book Record' : 'Add New Book to Inventory'}
                                        </h2>
                                        <p className="text-xs text-zinc-400 mt-0.5">Register new physical & digital title catalog entry</p>
                                    </div>
                                    {editingBook && (
                                        <button
                                ...
                                <form onSubmit={handleBookSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs font-semibold text-zinc-300 mb-1">ISBN Number</label>
                                        <input
                                            type="text"
                                            value={bookForm.isbn}
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold text-zinc-300 mb-1">Book Title</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold text-zinc-300 mb-1">Author Name</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold text-zinc-300 mb-1">Publisher</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold text-zinc-300 mb-1">Digital Purchase Price (KES)</label>
                                        <input
                                            type="number"
                                            step="0.01"
                                    ...
                                    <div className="sm:col-span-2">
                                        <label className="block text-xs font-semibold text-zinc-300 mb-1">Base64 PDF / File Path</label>
                                        <input
                                            type="text"
                            ...
                            {/* Books Catalog Table */}
                            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4 shadow-sm">
                                <h3 className="text-base font-bold text-zinc-100 font-mono">Catalog Books Inventory ({books.length})</h3>

                                <div className="overflow-x-auto">
                                    <table className="w-full text-left text-xs text-zinc-300 border-collapse">
                                        <thead className="bg-zinc-950 text-zinc-400 uppercase text-[10px] tracking-wider font-mono">
                                            <tr>
                                                <th className="p-3 border-b border-zinc-800">ISBN</th>
                                                <th className="p-3 border-b border-zinc-800">Title & Author</th>
                                                <th className="p-3 border-b border-zinc-800">Genre</th>
                                                <th className="p-3 border-b border-zinc-800 text-center">Copies (Total/Avail)</th>
                                                <th className="p-3 border-b border-zinc-800 text-center">Price</th>
                                                <th className="p-3 border-b border-zinc-800 text-right">Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-zinc-800/60">
                                            {books.map((b) => (
                                                <tr key={b.id} className="hover:bg-zinc-950/50">
                                                    <td className="p-3 font-mono font-bold text-zinc-300">{b.isbn}</td>
                                                    <td className="p-3">
                                                        <div className="font-bold text-zinc-100">{b.title}</div>
                                                        <div className="text-[11px] text-zinc-400">By {b.author}</div>
                                                    </td>
                                                    <td className="p-3 text-zinc-400 font-mono">{b.genre}</td>
                    ...
                    {activeTab === 'book_copies' && (
                        <div className="space-y-6">
                            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4 shadow-sm">
                                <h2 className="text-base font-bold text-zinc-100 flex items-center gap-2 font-mono">
                                    <Layers className="w-4 h-4 text-zinc-300" /> {editingCopy ? 'Edit Copy Record' : 'Register New Physical Book Copy'}
                                </h2>

                                <form onSubmit={handleCopySubmit} className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                    <div>
                                        <label className="block text-xs font-semibold text-zinc-300 mb-1">Select Book Title</label>
                                        <select
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold text-zinc-300 mb-1">Barcode</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold text-zinc-300 mb-1">Location Rack</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold text-zinc-300 mb-1">Availability Status</label>
                                        <select
                                    ...
                                    <div className="flex items-end gap-2">
                                        ...
                                        {editingCopy && (
                                            <button
                                                ...
                                                className="px-3 py-2 rounded-lg bg-zinc-800 text-zinc-300 text-xs hover:bg-zinc-700 h-9"
                                            >
                                                Cancel
                                            </button>
                                        )}
                            ...
                            {/* Book Copies Table */}
                            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4 shadow-sm">
                                <h3 className="text-base font-bold text-zinc-100 font-mono">Registered Physical Copies ({copies.length})</h3>

                                <div className="overflow-x-auto">
                                    <table className="w-full text-left text-xs text-zinc-300 border-collapse">
                                        <thead className="bg-zinc-950 text-zinc-400 uppercase text-[10px] tracking-wider font-mono">
                                            <tr>
                                                <th className="p-3 border-b border-zinc-800">Barcode</th>
                                                <th className="p-3 border-b border-zinc-800">Book Title</th>
                                                <th className="p-3 border-b border-zinc-800">Rack Location</th>
                                                <th className="p-3 border-b border-zinc-800 text-center">Condition</th>
                                        ...
                                        <tbody className="divide-y divide-zinc-800/60 font-mono">
                                            {copies.map((c) => (
                                                <tr key={c.id} className="hover:bg-zinc-950/50">
                                                    <td className="p-3 font-bold text-zinc-100">{c.barcode}</td>
                                                    <td className="p-3 font-sans font-semibold text-zinc-200">
                                                        {c.book ? c.book.title : `Book #${c.book_id}`}
                                                    </td>
                                                    <td className="p-3 text-zinc-400">{c.location_rack || 'N/A'}</td>
                                                    <td className="p-3 text-center capitalize">{c.condition}</td>
                                                    <td className="p-3 text-center">
                                                        <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${
                                                            c.status === 'available' ? 'bg-emerald-950 text-emerald-300 border border-emerald-800' : 'bg-zinc-800 text-zinc-300 border border-zinc-700'
                                                        }`}>
                                                            {c.status}
                                                        </span>
                                                    </td>
                    ...
                    {activeTab === 'returns' && (
                        <div className="space-y-6">
                            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4 shadow-sm">
                                <h2 className="text-base font-bold text-zinc-100 flex items-center gap-2 font-mono">
                                    <BookCheck className="w-4 h-4 text-zinc-300" /> Active Loans Directory & Instant Return Desk
                                </h2>
                                ...
                                {loading ? (
                                    ...
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-left text-xs text-zinc-300 border-collapse">
                                            <thead className="bg-zinc-950 text-zinc-400 uppercase text-[10px] tracking-wider font-mono">
                                                <tr>
                                                    <th className="p-3 border-b border-zinc-800">Loan ID</th>
                                                    <th className="p-3 border-b border-zinc-800">Member Info</th>
                                                    <th className="p-3 border-b border-zinc-800">Book Copy & Barcode</th>
                                                    <th className="p-3 border-b border-zinc-800 font-mono">Loan / Due Date</th>
                                                    <th className="p-3 border-b border-zinc-800 text-center">Status</th>
                                                    <th className="p-3 border-b border-zinc-800 text-right">Process Action</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-zinc-800/60 font-mono">
                                                {activeLoans.map((loan) => (
                                                    <tr key={loan.id} className="hover:bg-zinc-950/50">
                                                        ...
                                                        <td className="p-3 font-sans">
                                                            <div className="font-semibold text-zinc-100">{loan.member?.user?.name || `Member #${loan.member_id}`}</div>
                                                            <div className="text-[11px] font-mono text-zinc-400">{loan.member?.member_number}</div>
                                                        </td>
                                                        <td className="p-3 font-sans">
                                                            <div className="font-bold text-zinc-200">{loan.copy?.book?.title || 'Book Title'}</div>
                                                            <div className="text-[11px] font-mono text-zinc-400">{loan.copy?.barcode}</div>
                                                        </td>
                                                        <td className="p-3 text-zinc-300 text-[11px]">
                                                            <div>Loan: {loan.loan_date}</div>
                                                            <div className="text-zinc-400">Due: {loan.due_date}</div>
                                                        </td>
                                                        <td className="p-3 text-center">
                                                            <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${
                                                                loan.status === 'returned' ? 'bg-zinc-800 text-zinc-400 border border-zinc-700' : loan.status === 'overdue' ? 'bg-red-950 text-red-300 border border-red-800' : 'bg-emerald-950 text-emerald-300 border border-emerald-800'
                                                            }`}>
                                                                {loan.status}
                                                            </span>
                                                        </td>
                                                        <td className="p-3 text-right">
                                                            {loan.status !== 'returned' ? (
                                                                ...
                                                                <span className="text-[11px] text-zinc-500">Returned</span>
                                                            )}
                                                        </td>
                                                    </tr>
                                                ))}
                    ...
                    {activeTab === 'subscriptions' && (
                        <div className="space-y-6">
                            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4 shadow-sm">
                                <h2 className="text-base font-bold text-zinc-100 flex items-center gap-2 font-mono">
                                    <CreditCard className="w-4 h-4 text-zinc-300" /> {editingSub ? 'Edit Member Subscription' : 'Grant New Member Perk Subscription'}
                                </h2>

                                <form onSubmit={handleSubSubmit} className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                    <div>
                                        <label className="block text-xs font-semibold text-zinc-300 mb-1">Select Member</label>
                                        <select
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold text-zinc-300 mb-1">Subscription Plan Type</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold text-zinc-300 mb-1">Payment Amount (KES)</label>
                                        <input
                                            type="number"
                                            step="0.01"
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold text-zinc-300 mb-1">Expiration Date</label>
                                        <input
                                            type="date"
                            ...
                            {/* Subscriptions Table */}
                            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4 shadow-sm">
                                <h3 className="text-base font-bold text-zinc-100 font-mono">Valid Member Perk Subscriptions ({subscriptions.length})</h3>

                                <div className="overflow-x-auto">
                                    <table className="w-full text-left text-xs text-zinc-300 border-collapse">
                                        <thead className="bg-zinc-950 text-zinc-400 uppercase text-[10px] tracking-wider font-mono">
                                            <tr>
                                                <th className="p-3 border-b border-zinc-800">Sub ID / Reference</th>
                                                <th className="p-3 border-b border-zinc-800">Member</th>
                                                <th className="p-3 border-b border-zinc-800">Plan Type</th>
                                                <th className="p-3 border-b border-zinc-800 text-center">Discount</th>
                                                <th className="p-3 border-b border-zinc-800 text-center">Amount Paid</th>
                                                <th className="p-3 border-b border-zinc-800 text-center">Status</th>
                                                <th className="p-3 border-b border-zinc-800 font-mono">Expires At</th>
                                                <th className="p-3 border-b border-zinc-800 text-right">Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-zinc-800/60 font-mono">
                                            {subscriptions.map((s) => (
                                                <tr key={s.id} className="hover:bg-zinc-950/50">
                                                    <td className="p-3 font-bold text-zinc-200">
                                                        #{s.id}
                                                        <div className="text-[10px] text-zinc-500 font-mono">{s.transaction_reference || 'REF-N/A'}</div>
                                                    </td>
                                                    <td className="p-3 font-sans">
                                                        <div className="font-semibold text-zinc-100">{s.member?.user?.name || s.user?.name || `Member #${s.member_id}`}</div>
                                                        <div className="text-[11px] font-mono text-zinc-400">{s.member?.member_number}</div>
                                                    </td>
                                                    <td className="p-3 text-zinc-300">{s.plan_type}</td>
                                                    <td className="p-3 text-center text-zinc-300">{s.discount_percentage}% OFF</td>
                                                    <td className="p-3 text-center font-bold text-zinc-100">KES {s.amount_paid}</td>
                                                    <td className="p-3 text-center">
                                                        <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${
                                                            s.payment_status === 'paid' ? 'bg-emerald-950 text-emerald-300 border border-emerald-800' : 'bg-zinc-800 text-zinc-400 border border-zinc-700'
                                                        }`}>
                                                            {s.payment_status}
                                                        </span>
                                                    </td>
