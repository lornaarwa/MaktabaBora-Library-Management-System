# Smart Library System (SmartLib) - Visual Diagrams Gallery

This directory contains standalone, high-resolution PNG image exports of all architectural and UML diagrams for the **Smart Library Management System**.

---

## 1. Use Case Diagram
### Actor Interactions & Functional Scope
![Use Case Diagram](use_case_diagram.png)

---

## 2. Sequence Diagrams

### Sequence 1: Authentication & JWT Token Lifecycle
![Authentication & JWT Token Lifecycle](sequence_01_auth_jwt_lifecycle.png)

---

### Sequence 2: Physical Book Circulation (Checkout, Returns & Overdue Fines)
![Physical Book Circulation](sequence_02_physical_circulation_fines.png)

---

### Sequence 3: Hold Reservation Queue & Staff Fulfillment
![Hold Reservation Queue](sequence_03_hold_reservation_queue.png)

---

### Sequence 4: Digital Book Purchase via M-Pesa STK Push & Lifetime Reader
![Digital Book Purchase via M-Pesa STK Push](sequence_04_digital_purchase_mpesa_reader.png)

---

### Sequence 5: Membership Perk Pass Subscription & Refund Flow
![Membership Perk Pass Subscription & Refund Flow](sequence_05_membership_perks_refund.png)

---

### Sequence 6: Smart AI Librarian Assistant (Rate-Limited RAG Flow)
![Smart AI Librarian Assistant](sequence_06_ai_librarian_rag_assistant.png)

---

## 3. Class Diagrams

### Class Diagram 1: Eloquent Models Architecture (14 Models)
![Eloquent Models Architecture](class_diagram_01_models.png)

---

### Class Diagram 2: REST Controllers Layer (15 Controllers)
![REST Controllers Layer](class_diagram_02_controllers.png)

---

### Class Diagram 3: Domain Services & Contracts Layer
![Domain Services & Contracts Layer](class_diagram_03_services.png)

---

## 4. Relational Database Schema

### Database Schema: Entity-Relationship (ER) Diagram
![Database Entity-Relationship (ER) Diagram](database_schema_er_diagram.png)
